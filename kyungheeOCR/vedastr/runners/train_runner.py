...
class TrainRunner(InferenceRunner):
    ...
    def _validate_batch(self, img, label):
        ...
        with torch.no_grad():
            ...
            # Measure Metrics (Accuracy, 1-NED)
    ...
    def _train_batch(self, img, label):
        ...
        if self.iter != 0 and self.iter % self.log_interval == 0:
            self.logger.info(
                ...
                (self.epoch, self.iter, self.lr, loss.item(), self.metric.avg['acc']['true'], self.metric.avg['edit']))
    ...
    def __call__(self):
        ...
        while flag:
            for img, label in self.train_dataloader:
                ...
                # Learning Rate Update (iteration based)
                self.lr_scheduler.iter_nums()
                if iter_based:
                    self.lr_scheduler.step()
                elif warmup_iters > 0 and warmup_iters >= self.iter:
                    self.lr_scheduler.step()

                # Validate the model in epoch
                if self.trainval_ratio > 0 and (self.iter + 1) % self.trainval_ratio == 0 and self.val_dataloader:
            ...
            # Learning Rate Update (epoch based)
            if not iter_based:
                self.lr_scheduler.step()
    ...
    def save_model(self, out_dir, filename, save_optimizer=True, meta=None):
        current_meta = dict(epoch=self.epoch, iter=self.iter, lr=self.lr, best_acc=self.best_acc, best_norm=self.best_norm)
        if meta is None:
            meta = current_meta
        else:
    ...
    def resume(self, checkpoint, resume_optimizer=False, resume_lr_scheduler=False, resume_meta=False, strict=True, map_location='default'):
        checkpoint = self.load_checkpoint(checkpoint, map_location=map_location, strict=strict)
