...
class CLAHE(albu.CLAHE):
    def __init__(self, 
                 ...
                 always_apply=True, ):
        super(CLAHE, self).__init__(clip_limit, tile_grid_size, always_apply=always_apply)
...
@TRANSFORMS.register_module
class MedianBlur(albu.MedianBlur):
    def __init__(self, **kwargs):
        super(MedianBlur, self).__init__(always_apply = True)
    def apply(self, img: np.ndarray, ksize: int = 3, **params) -> np.ndarray:
        return FB.median_blur(img, ksize=5)
...
class Rotate(albu.Rotate):
    def __init__(self, limit=90,
                 ...
                 always_apply=False,
                 p=0.5, ):
        super(Rotate, self).__init__(limit, CV2_INTER[interpolation], CV2_BORDER[border_mode], value=value, always_apply=always_apply, p=p)
